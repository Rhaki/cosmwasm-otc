...
pub fn test() {
    ...
    let ask_items = vec![
        ...
        OtcItemRegistration { info: OtcItemInfo::Cw721 { contract: ask_nft_addr.clone(), token_id: ask_nft_id.to_string() }, vesting: None }
    ];
    ...
    run_create_otc(&mut app, &mut def, creator, dealer, &offer_items, &ask_items, fee.clone()).unwrap();
    ...
    increase_allowance(&mut app, dealer, &def.addr_otc.clone().unwrap().to_string(), &ask_nft_addr, TokenType::Cw721, ask_nft_id);
    increase_allowance(&mut app, dealer, &def.addr_otc.clone().unwrap().to_string(), &ask_cw20_addr, TokenType::Cw20, &ask_cw20_amount.to_string());
    mint_token(&mut app, &mut def, dealer, (&fee[0].denom, TokenType::Native), &fee[0].amount.to_string());

    run_execute_otc(&mut app, &mut def, dealer, 1, vec![]).unwrap_err();
    run_execute_otc(&mut app, &mut def, dealer, 1, fee.clone()).unwrap();

    // assert result

    assert_eq!(offer_cw20_amount, qy_balance_cw20(&app, &offer_cw20_addr, dealer).u128());
    assert_eq!(offer_native_amount, qy_balance_native(&app, &offer_native_denom, dealer).u128());
    assert_eq!(true, qy_balance_nft(&app, &offer_nft_addr, offer_nft_id, dealer));
    ...
    assert_eq!(true, qy_balance_nft(&app, &ask_nft_addr, ask_nft_id, creator));
